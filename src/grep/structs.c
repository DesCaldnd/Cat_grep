...
struct String init_string_from_stream_buf(FILE* stream, char* buf,
  ...
  fscanf(stream, "%c", &c);
  while (!is_needed_sym(c) && c != EOF && c != '\0' && !no_skip_sym(c) &&
         !feof(stream))
    fscanf(stream, "%c", &c);
...
int is_empty_string(struct String* data) { return data->size <= 0; }
...
void push_string_c(struct String* data, char c) {
  if (data->size > data->capacity - 2) realloc_string(data, data->capacity * 2);
...
void cat_string(struct String* dest, const struct String* src) {
  if (dest->size + src->size > dest->capacity - 1) {
    realloc_string(dest, dest->size + src->size +
