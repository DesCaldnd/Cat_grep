...
#define LIST(type, st_type)                                                   \
  ...
  int is_list_empty_##type(struct list_##type list) {                         \
    return list.begin->next == NULL;                                          \
                                                                              ...
                                                                              \
  struct node_##type* insert_list_##type(struct node_##type* node,            \
                                         st_type data) {                      \
  ...
  struct list_##type copy_list_##type(const struct list_##type src) {         \
    ...
    if (!is_list_valid_##type(result)) return result;                         \
                                                                              \
  ...
  int size_list_##type(struct list_##type list) {                             \
    ...
    return res;                                                               \
                                                                              ...
                                                                              \
  struct node_##type* find_list_##type(struct list_##type list, st_type data, \
                                       int (*cmpf)(st_type, st_type)) {       \
    struct node_##type* node = list.begin->next;                              \
    ...
    return node;                                                              \
                                                                              ...
                                                                              \
  void for_all_list_##type(struct list_##type list,                           \
                           void (*func)(st_type*, void*), void* opt_data) {   \
    struct node_##type* node = list.begin->next;                              \
    while (node != NULL) {                                                    \
...
#define VECTOR(type, st_type)                                                 \
  ...
  int is_valid_vector_##type(struct vector_##type* vector) {                  \
    return vector->data != NULL;                                              \
                                                                              ...
                                                                              \
  void for_all_vector_##type(struct vector_##type* vector,                    \
                             void (*func)(st_type*, void*), void* opt_data) { \
  ...
  void destroy_vector_##type(struct vector_##type* vector) {                  \
    ...
    vector->data = NULL;                                                      \
                                                                              ...
                                                                              \
  void insert_vector_##type(struct vector_##type* vector, st_type data,       \
                            size_t ind, void (*deleter)(st_type*, void*)) {   \
    if (vector->size >= vector->capacity)                                     \
      realloc_vector_##type(vector, vector->capacity * 2, deleter);           \
                                                                              \
    if (is_valid_vector_##type(vector)) {                                     \
      if (ind > vector->size) ind = vector->size;                             \
      st_type tmp;                                                            \
  ...
  void remove_vector_##type(struct vector_##type* vector, size_t ind) {       \
    ...
    --vector->size;                                                           \
                                                                              ...
                                                                              \
  st_type* find_vector_##type(struct vector_##type* vector, st_type data,     \
                              int (*comp)(st_type, st_type)) {                \
    st_type* res = NULL;                                                      \
    for (size_t i = 0; i < vector->size; ++i) {                               \
...
struct String init_string_from_stream_buf(FILE* stream, char* buf,
                                          int (*is_needed_sym)(int),
                                          int (*no_skip_sym)(int));

struct String init_string_from_stream_no_skip(FILE* stream,
                                              int (*is_needed_sym)(int));
...
#define DEQUE(type, st_type)                                                 \
  ...
  st_type* get_deque_##type(struct deque_##type* deque, size_t index) {      \
    return &deque->data[(index + deque->start) % deque->capacity];           \
                                                                             ...
                                                                             \
  void for_all_deque_##type(struct deque_##type* deque,                      \
                            void (*func)(st_type*, void*), void* opt_data) { \
  ...
  void destroy_deque_##type(struct deque_##type* deque) {                    \
    ...
    deque->capacity = 0;                                                     \
                                                                             ...
                                                                             \
  void realloc_deque_##type(struct deque_##type* deque, size_t size,         \
                            void (*deleter)(st_type*, void*)) {              \
    st_type* tmp = malloc(sizeof(st_type) * size);                           \
    if (tmp == NULL) {                                                       \
  ...
  int is_empty_deque_##type(struct deque_##type* deque) {                    \
    return deque->size == 0;                                                 \
                                                                             ...
                                                                             \
  int push_front_deque_##type(struct deque_##type* deque, st_type data,      \
                              void (*deleter)(st_type*, void*)) {            \
    ...
    return 0;                                                                \
                                                                             ...
                                                                             \
  int push_back_deque_##type(struct deque_##type* deque, st_type data,       \
                             void (*deleter)(st_type*, void*)) {             \
    if (deque->capacity <= deque->size + 1) {                                \
      realloc_deque_##type(deque, deque->size * 2, deleter);                 \
    }                                                                        \
...
#define PAIR(f_type, fs_type, s_type, ss_type) \
  struct pair_##f_type##s_type {               \
    fs_type first;                             \
    ss_type second;                            \
  };
...
#define UNORDERED_MAP(k_type, ks_type, v_type, vs_type)                       \
  struct map_data_##k_type##v_type {                                          \
  ...
  struct map_##k_type##v_type {                                               \
    ...
    int (*eq)(ks_type, ks_type);                                              \
                                                                              ...
                                                                              \
  struct map_##k_type##v_type init_map_##k_type##v_type(                      \
      int (*eq)(ks_type, ks_type)) {                                          \
    struct map_##k_type##v_type map;                                          \
    map.buckets.data = NULL;                                                  \
    map.data = init_list_ml_##k_type##v_type();                               \
    if (!is_list_valid_ml_##k_type##v_type(map.data)) return map;             \
                                                                              \
  ...
  int is_valid_map_##k_type##v_type(struct map_##k_type##v_type* map) {       \
    return is_valid_vector_mv_##k_type##v_type(&map->buckets) &&              \
           is_list_valid_ml_##k_type##v_type(map->data) &&                    \
  ...
  int need_rehash_map_##k_type##v_type(struct map_##k_type##v_type* map) {    \
    return ((double)map->size / (double)map->bucket_count) >                  \
               ...
               : 0;                                                           \
                                                                              ...
                                                                              \
  void rehash_map_##k_type##v_type(                                           \
      struct map_##k_type##v_type* map,                                       \
      void (*deleter)(struct pair_##k_type##v_type*)) {                       \
    ...
    map->bucket_count = 0;                                                    \
                                                                              \
    while (it != NULL) {                                                      \
      struct node_ml_##k_type##v_type* node_before =                          \
          map->buckets.data[it->data.hash % size];                            \
      ...
      if (need_increase) ++map->bucket_count;                                 \
                                                                              \
      it = next;                                                              \
      if (next != NULL) next = next->next;                                    \
                                                                              ...
                                                                              \
  struct node_ml_##k_type##v_type* find_node_map_##k_type##v_type(            \
      struct map_##k_type##v_type* map, ks_type key, int hash) {              \
    size_t size = map->buckets.size;                                          \
    struct node_ml_##k_type##v_type* res = map->buckets.data[hash % size];    \
    int found = 0;                                                            \
                                                                              \
    while (res->next != NULL && hash % size == res->next->data.hash % size && \
           !found) {                                                          \
      if (res->next->data.hash == hash &&                                     \
          map->eq(key, res->next->data.pair.first))                           \
        found = 1;                                                            \
      else                                                                    \
        res = res->next;                                                      \
    }                                                                         \
    return found ? res : NULL;                                                \
                                                                              ...
                                                                              \
  struct pair_##k_type##v_type* find_map_##k_type##v_type(                    \
      struct map_##k_type##v_type* map, ks_type key) {                        \
    int hash = hash_##k_type(key);                                            \
    struct node_ml_##k_type##v_type* res =                                    \
        find_node_map_##k_type##v_type(map, key, hash);                       \
    return (res == NULL)       ? NULL                                         \
           : res->next == NULL ? NULL                                         \
                               : &res->next->data.pair;                       \
  }                                                                           \
                                                                              \
  int insert_map_##k_type##v_type(                                            \
      struct map_##k_type##v_type* map, ks_type key, vs_type value,           \
      void (*deleter)(struct pair_##k_type##v_type*)) {                       \
    int hash = hash_##k_type(key);                                            \
    struct node_ml_##k_type##v_type* it =                                     \
        find_node_map_##k_type##v_type(map, key, hash);                       \
                                                                              \
    if (it != NULL && it->next != NULL) {                                     \
      ...
      struct node_ml_##k_type##v_type* inserted_node =                        \
          insert_list_ml_##k_type##v_type(it, data);                          \
      if (inserted_node == NULL) return 0;                                    \
                                                                              \
      ++map->size;                                                            \
      int need_increase = 0;                                                  \
      ++ind;                                                                  \
    ...
    return 1;                                                                 \
                                                                              ...
                                                                              \
  void remove_map_##k_type##v_type(                                           \
      struct map_##k_type##v_type* map, ks_type key,                          \
      void (*deleter)(struct pair_##k_type##v_type*)) {                       \
    int hash = hash_##k_type(key);                                            \
    struct node_ml_##k_type##v_type* it =                                     \
    ...
    if (it != NULL && it->next != NULL) {                                     \
      ...
      while (ind < map->buckets.size && map->buckets.data[ind] == it->next) { \
        ...
        ++ind;                                                                \
                                                                              ...
                                                                              \
      --map->size;                                                            \
      if (need_decrease) --map->bucket_count;                                 \
      ...
      if (need_rehash_map_##k_type##v_type(map))                              \
        rehash_map_##k_type##v_type(map, deleter);                            \
                                                                              ...
                                                                              \
  void set_max_load_factor(struct map_##k_type##v_type* map, double factor,   \
                           void (*deleter)(struct pair_##k_type##v_type*)) {  \
